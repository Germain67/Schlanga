...
direction IA_defensif_direction(serpent schlanga, plateau p) {
	...
	if (count[0]==-1 && count[1]==-1 && count[2]==-1 && count[3]==-1) { // Le schlanga ne peut plus se déplacer, il a perdu, il nous suffit de lui donner une direction aléatoire où il va se suicider
	dir=GAUCHE; 
	}
	else {
